<x-app-layout>
    <div
        class="max-w-2xl border border-gray-800 mx-auto backdrop-blur-sm  text-gray-200 p-6 rounded-xl mt-10 shadow-md">
        ...
        <p><strong>Type:</strong> {{ $rock->type->type }}</p>
        <p><strong>Category:</strong> {{ $rock->category->category }}</p>
        <p><strong>Color:</strong> {{ $rock->color->color }}</p>
        <p><strong>Hardness:</strong> {{ $rock->hardness_id }}</p>
        ...
        <div class="flex flex-col sm:flex-row sm:space-x-3 space-y-2 sm:space-y-0">
            <a href="/rocks/{{ $rock->id }}/edit"
               class="flex-1 text-center bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg px-4 py-2 transition">Edit</a>
        ...
        <div class="flex flex-col max-w-2xl mx-auto bg-gray-900 text-gray-200 p-6 rounded-xl mt-10 shadow-md">
            ...
            <div class=" flex-col mb-4">
                <form method="POST" action="{{ route('comments.store', $rock) }}">
                    @csrf
                    <textarea class="bg-gray-800 rounded" name="comment" rows="3"
            ...
            <div class="text-white flex flex-col space-y-4">
                @foreach($rock->comments as $comment)
                    <p>{{ ($comment->comment) }} </p>
                    <p class="text-sm text-gray-500">Posted by {{ $comment->user->name ?? 'Unknown' }}
                        on {{ $comment->created_at->format('d M Y H:i') }}</p>
